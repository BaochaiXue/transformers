        ...
        *,
        ...
        Args:
            ...
            prefix (`str` or token IDs, *optional*):
                ...
                `response_template` specs; ignored for legacy `response_schema`.
        """
        ...
        if prefix is not None:
            if not use_new_template:
                raise ValueError(
                    "`prefix=` is only supported with new-style `response_template` specs, "
                    "not legacy `response_schema`."
                )
            prefix_str = prefix if isinstance(prefix, str) else self.decode(prefix)

        if use_new_template:
            parse_one = lambda text: _template_parse_response(text, schema, prefix=prefix_str)  # noqa: E731
        else:
        ...
        if batched:
            ...
            return [parse_one(single_response) for single_response in response]
        else:
